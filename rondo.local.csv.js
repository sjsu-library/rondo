...
//     if (row.c[8]) $('.modal-image').html('<img src="'+get(8)+'" alt="'+title+'">');
//     else if (row.c[5]) $('.modal-image').html('<img src="'+get(5)+'" alt="'+title+'">');
...
//     // Home: start with no prefilter; Page view: prefilter by current page’s Keywords
//     let initialSearch = {"columns":[0,1,2,3,4,7,10,11,12,13,14,15,16,17]};
//     if (openPage){
//       const q = $('article.'+openPage).attr('pagequery');
//       if (q){
//         initialSearch = {"preDefined": { "criteria": [ { "condition": "contains", "data": "Keywords", "type": "string", "value": [ q ] } ], "logic": "AND" }, "columns":[0,1,2,3,4,7,10,11,12,13,14,15,16,17]};
//       }
//     }
...
//           api.search('').searchBuilder.rebuild().draw();
...
//     const articleTitle = $(event.target).attr('pagetitle');

//     $('#pages-container article').hide();
//     $('article.'+slug).show();

//     const storedSearch = { "criteria": [ { "condition": "contains", "data": "Keywords", "type": "string", "value": [ query ] } ], "logic": "AND" };
//     if (itemsTable){ itemsTable.search('').searchBuilder.rebuild(storedSearch).draw(); }
...
function updateToolsPanel(){
  ...
  async function boot(){
    ...
    //   $('.tools-pages').text(JSON.stringify(pagesJson));
    //   $('.tools-items').text(JSON.stringify(itemsJson));
      // Open the Tools modal and (re)populate JSON
